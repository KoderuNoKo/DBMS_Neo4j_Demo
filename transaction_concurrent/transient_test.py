...
def inc_view_ab(tx):
    tx.run("""
        MATCH (a:Movie {movieId:1}), (b:Movie {movieId:2})
        SET a.viewCount = coalesce(a.viewCount,0) + 1
        WITH b
        CALL apoc.util.sleep(2000)
        SET b.viewCount = coalesce(b.viewCount,0) + 1;
    """)
    
def inc_view_ba(tx):
    tx.run("""
        MATCH (a:Movie {movieId:2}), (b:Movie {movieId:1})
        SET a.viewCount = coalesce(a.viewCount,0) + 1
        WITH b
        CALL apoc.util.sleep(2000)
        SET b.viewCount = coalesce(b.viewCount,0) + 1;
...
def worker(thread_id, runs_num, shared_session=None):
    ...
    for i in range(runs_num):
        ...
        try:
            if thread_id % 2 == 0:
                local_session.execute_write(inc_view_ab)
            else:
                local_session.execute_write(inc_view_ba)
            status = "OK"
